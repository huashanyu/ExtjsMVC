...
Ext.define("Ext.draw.CompositeSprite", {extend: "Ext.util.MixedCollection", mixins: {animate: "Ext.util.Animate"}, autoDestroy: false, isCompositeSprite: true, constructor: function (a) {
    var b = this;
    Ext.apply(b, a);
...
Ext.define("Ext.draw.Surface", {mixins: {observable: "Ext.util.Observable"}, requires: ["Ext.draw.CompositeSprite"], uses: ["Ext.draw.engine.Svg", "Ext.draw.engine.Vml", "Ext.draw.engine.SvgExporter", "Ext.draw.engine.ImageExporter"], separatorRe: /[, ]+/, enginePriority: ["Svg", "Vml"], statics: {create: function (b, d) {
    ...
    b = b || {};
    var e = {"image/png": "Image", "image/jpeg": "Image", "image/svg+xml": "Svg"}, d = e[b.type] || "Svg", c = Ext.draw.engine[d + "Exporter"];
    ...
    if (b) {
        if (b.gradient) {
            ...
            e = f.id;
            d.addGradient(f);
            d.background = d.add({type: "rect", isBackground: true, x: 0, y: 0, width: c, height: a, fill: "url(#" + e + ")", zIndex: -1})
        } else {
            if (b.fill) {
                d.background = d.add({type: "rect", isBackground: true, x: 0, y: 0, width: c, height: a, fill: b.fill, zIndex: -1})
            } else {
                if (b.image) {
                    d.background = d.add({type: "image", isBackground: true, x: 0, y: 0, width: c, height: a, src: b.image, zIndex: -1})
                }
            }
        }
    ...
    if (l && (a || g)) {
        ...
        if (d.background) {
            d.background.setAttributes(Ext.apply({}, {x: f, y: e, width: a / k, height: g / k}, {hidden: false}), true)
        }
    } else {
        if (d.background && a && g) {
...
Ext.define("Ext.draw.layout.Component", {alias: "layout.draw", extend: "Ext.layout.component.Auto", setHeightInDom: true, setWidthInDom: true, type: "draw", measureContentWidth: function (b) {
...
Ext.define("Ext.draw.Component", {alias: "widget.draw", extend: "Ext.Component", requires: ["Ext.draw.Surface", "Ext.draw.layout.Component"], enginePriority: ["Svg", "Vml"], baseCls: Ext.baseCSSPrefix + "surface", componentLayout: "draw", viewBox: true, shrinkWrap: 3, autoSize: false, suspendSizing: 0, onRender: function () {
    this.callParent(arguments);
    ...
    var d = this, b = Ext.applyIf({renderTo: d.el, height: d.height, width: d.width, items: d.items}, d.initialConfig), a;
    delete b.listeners;
    if (!b.gradients) {
    ...
    a.on({scope: d, mouseup: c("mouseup"), mousedown: c("mousedown"), mousemove: c("mousemove"), mouseenter: c("mouseenter"), mouseleave: c("mouseleave"), click: c("click"), dblclick: c("dblclick")})
}, initSurfaceCfg: Ext.emptyFn, onDestroy: function () {
    Ext.destroy(this.surface);
    this.callParent(arguments)
}});
Ext.define("Ext.rtl.draw.Component", {override: "Ext.draw.Component", initSurfaceCfg: function (a) {
...
Ext.define("Ext.draw.Color", {colorToHexRe: /(.*?)rgb\((\d+),\s*(\d+),\s*(\d+)\)/, rgbRe: /\s*rgb\s*\(\s*([0-9]+)\s*,\s*([0-9]+)\s*,\s*([0-9]+)\s*\)\s*/, hexRe: /\s*#([0-9a-fA-F][0-9a-fA-F]?)([0-9a-fA-F][0-9a-fA-F]?)([0-9a-fA-F][0-9a-fA-F]?)\s*/, lightnessFactor: 0.2, constructor: function (d, c, a) {
    ...
    return new Ext.draw.Color(j[0] * 255, j[1] * 255, j[2] * 255)
}}, function () {
...
Ext.define("Ext.chart.theme.Theme", (function () {
    ...
    return{requires: ["Ext.draw.Color"], theme: "Base", themeAttrs: false, initTheme: function (e) {
...
Ext.define("Ext.chart.MaskLayer", {extend: "Ext.Component", constructor: function (a) {
...
Ext.define("Ext.chart.Mask", {mixinId: "mask", requires: ["Ext.chart.MaskLayer"], constructor: function (a) {
    var b = this;
    if (a) {
        Ext.apply(b, a)
    }
    if (b.enableMask) {
        b.on("afterrender", function () {
            ...
            b.maskType = b.mask;
            b.mask = c;
            b.maskSprite = b.surface.add({type: "path", path: ["M", 0, 0], zIndex: 1001, opacity: 0.6, hidden: true, stroke: "#00f", cursor: "crosshair"})
        }, b, {single: true})
    }
    ...
    if (d.type === "mousedown") {
        ...
        if (f && g.mouseDown) {
            ...
            g.maskSelection = {x: (a > 0 ? p : p + a) + g.el.getX(), y: (n > 0 ? m : m + n) + g.el.getY(), width: r(a), height: r(n)};
...
Ext.define("Ext.chart.Navigation", {mixinId: "navigation", setZoom: function (o) {
    var s = this, n = s.axes.items, q, l, c, a = s.chartBBox, t = a.width, d = a.height, f = {x: o.x - s.el.getX(), y: o.y - s.el.getY(), width: o.width, height: o.height}, h, m, p, b, g, k, j, e, r;
...
Ext.define("Ext.chart.LegendItem", {extend: "Ext.draw.CompositeSprite", requires: ["Ext.chart.Shape"], hiddenSeries: false, label: undefined, x: 0, y: 0, zIndex: 500, boldRe: /bold\s\d{1,}.*/i, constructor: function (a) {
    this.callParent(arguments);
    this.createLegend(a)
    ...
    return c.add("label", c.surface.add({type: "text", x: 20, y: 0, zIndex: (c.zIndex || 0) + 2, fill: b.labelColor, font: b.labelFont, text: c.getLabelText(), style: {cursor: "pointer"}}))
    ...
    return e.add("line", a.add({type: "path", path: "M" + h + "," + f + "L" + j + "," + g, zIndex: (d || 0) + 2, "stroke-width": b.lineWidth, "stroke-linejoin": "round", "stroke-dasharray": b.dash, stroke: k.stroke || b.getLegendColor(c) || "#000", style: {cursor: "pointer"}}))
}, drawMarker: function (b, g, f, e) {
    var d = this, a = d.surface, c = d.series;
    return d.add("marker", Ext.chart.Shape[e.type](a, {fill: e.fill, x: b, y: g, zIndex: (f || 0) + 2, radius: e.radius || e.size, style: {cursor: "pointer"}}))
}, drawFilledBox: function (e, b, g, c) {
    var f = this, a = f.surface, d = f.series;
    return f.add("box", a.add({type: "rect", zIndex: (g || 0) + 2, x: 0, y: 0, width: e, height: b, fill: d.getLegendColor(c), style: {cursor: "pointer"}}))
}, onMouseOver: function () {
    var a = this;
    a.label.setStyle({"font-weight": "bold"});
    a.series._index = a.yFieldIndex;
    a.series.highlightItem()
}, onMouseOut: function () {
    var b = this, a = b.legend, c = b.boldRe;
    b.label.setStyle({"font-weight": a.labelFont && c.test(a.labelFont) ? "bold" : "normal"});
...
Ext.define("Ext.rtl.chart.LegendItem", {override: "Ext.chart.LegendItem", updatePosition: function (f) {
...
Ext.define("Ext.chart.Legend", {requires: ["Ext.chart.LegendItem"], visible: true, update: true, position: "bottom", x: 0, y: 0, labelColor: "#000", labelFont: "12px Helvetica, sans-serif", boxStroke: "#000", boxStrokeWidth: 1, boxFill: "#FFF", itemSpacing: 10, padding: 5, width: 0, height: 0, boxZIndex: 100, constructor: function (a) {
    var b = this;
    if (a) {
        Ext.apply(b, a)
    }
...
}, init: Ext.emptyFn, redraw: function () {
    var a = this;
    a.create();
    a.updatePosition()
    ...
    var a = this;
    return{x: Math.round(a.x) - a.boxStrokeWidth / 2, y: Math.round(a.y) - a.boxStrokeWidth / 2, width: a.width + a.boxStrokeWidth, height: a.height + a.boxStrokeWidth}
}, createBox: function () {
    var b = this, a, c;
    if (b.boxSprite) {
        b.boxSprite.destroy()
    ...
    a = b.boxSprite = b.chart.surface.add(Ext.apply({type: "rect", stroke: b.boxStroke, "stroke-width": b.boxStrokeWidth, fill: b.boxFill, zIndex: b.boxZIndex}, c));
    a.redraw()
...
Ext.define("Ext.rtl.chart.Legend", {override: "Ext.chart.Legend", init: function () {
    var a = this;
...
Ext.define("Ext.chart.theme.Base", {requires: ["Ext.chart.theme.Theme"], constructor: function (a) {
    ...
    Ext.chart.theme.call(this, a, {background: false, axis: {stroke: "#444", "stroke-width": 1}, axisLabelTop: {fill: "#444", font: "12px Arial, Helvetica, sans-serif", spacing: 2, padding: 5, renderer: b}, axisLabelRight: {fill: "#444", font: "12px Arial, Helvetica, sans-serif", spacing: 2, padding: 5, renderer: b}, axisLabelBottom: {fill: "#444", font: "12px Arial, Helvetica, sans-serif", spacing: 2, padding: 5, renderer: b}, axisLabelLeft: {fill: "#444", font: "12px Arial, Helvetica, sans-serif", spacing: 2, padding: 5, renderer: b}, axisTitleTop: {font: "bold 18px Arial", fill: "#444"}, axisTitleRight: {font: "bold 18px Arial", fill: "#444", rotate: {x: 0, y: 0, degrees: 270}}, axisTitleBottom: {font: "bold 18px Arial", fill: "#444"}, axisTitleLeft: {font: "bold 18px Arial", fill: "#444", rotate: {x: 0, y: 0, degrees: 270}}, series: {"stroke-width": 0}, seriesLabel: {font: "12px Arial", fill: "#333"}, marker: {stroke: "#555", radius: 3, size: 3}, colors: ["#94ae0a", "#115fa6", "#a61120", "#ff8809", "#ffd13e", "#a61187", "#24ad9a", "#7c7474", "#a66111"], seriesThemes: [
        ...
        {fill: "#115fa6", type: "plus"},
        {fill: "#94ae0a", type: "circle"},
        {fill: "#a61120", type: "cross"}
    ]})
}}, function () {
    ...
    for (; g < b; g++) {
        a[h[g]] = (function (j) {
            return Ext.extend(a.Base, {constructor: function (k) {
...
Ext.define("Ext.chart.Chart", {extend: "Ext.draw.Component", alias: "widget.chart", mixins: ["Ext.chart.theme.Theme", "Ext.chart.Mask", "Ext.chart.Navigation", "Ext.util.StoreHolder", "Ext.util.Observable"], uses: ["Ext.chart.series.Series"], requires: ["Ext.util.MixedCollection", "Ext.data.StoreManager", "Ext.chart.Legend", "Ext.chart.theme.Base", "Ext.chart.theme.Theme", "Ext.util.DelayedTask"], viewBox: false, animate: false, legend: false, insetPadding: 10, background: false, refreshBuffer: 1, constructor: function (b) {
    var c = this, a;
    b = Ext.apply({}, b);
    ...
    b.on({mousemove: b.onMouseMove, mouseleave: b.onMouseLeave, mousedown: b.onMouseDown, mouseup: b.onMouseUp, click: b.onClick, dblclick: b.onDblClick, scope: b})
    ...
    var j = this, h = j.series.items, f = h.length, b = j.axes.items, d = b.length, a = 0, g, m, l = j.chartBBox = {x: 0, y: 0, height: j.curHeight, width: j.curWidth}, k = j.legend, e;
    ...
    b.initialized = true
}, configureAxisStyles: Ext.emptyFn, getInsets: function () {
    ...
    Ext.destroy(b.surface);
    b.bindStore(null);
    b.callParent(arguments)
}});
Ext.define("Ext.rtl.chart.Chart", {override: "Ext.chart.Chart", initSurfaceCfg: function (a) {
    this.callParent(arguments);
    a.forceLtr = true
...
Ext.define("Ext.chart.Highlight", {requires: ["Ext.fx.Anim"], highlight: false, highlightCfg: {fill: "#fdd", "stroke-width": 5, stroke: "#f55"}, constructor: function (a) {
...
Ext.define("Ext.chart.Label", {requires: ["Ext.draw.Color"], colorStringRe: /url\s*\(\s*#([^\/)]+)\s*\)/, constructor: function (a) {
    var b = this;
    b.label = Ext.applyIf(b.label || {}, {display: "none", stackedDisplay: "none", color: "#000", field: "name", minMargin: 50, font: "11px Helvetica, sans-serif", orientation: "horizontal", renderer: Ext.identityFn});
...
Ext.define("Ext.chart.Tip", {requires: ["Ext.tip.ToolTip", "Ext.chart.TipSurface"], constructor: function (b) {
    ...
    if (b.tips) {
        c.tipTimeout = null;
        c.tipConfig = Ext.apply({}, b.tips, {renderer: Ext.emptyFn, constrainPosition: true, autoHide: true, shrinkWrapDock: true});
...
Ext.define("Ext.draw.Draw", {singleton: true, requires: ["Ext.draw.Color"], pathToStringRE: /,?([achlmqrstvxz]),?/gi, pathCommandRE: /([achlmqstvz])[\s,]*((-?\d*\.?\d*(?:e[-+]?\d+)?\s*,?\s*)+)/ig, pathValuesRE: /(-?\d*\.?\d*(?:e[-+]?\d+)?)\s*,?\s*/ig, stopsRE: /^(\d+%?)$/, radian: Math.PI / 180, availableAnimAttrs: {along: "along", blur: null, "clip-rect": "csv", cx: null, cy: null, fill: "color", "fill-opacity": null, "font-size": null, height: null, opacity: null, path: "path", r: null, rotation: "csv", rx: null, ry: null, scale: "csv", stroke: "color", "stroke-opacity": null, "stroke-width": null, translation: "csv", width: null, x: null, y: null}, is: function (b, a) {
    ...
    var e = 1 - k;
    return{x: Math.pow(e, 3) * b + Math.pow(e, 2) * 3 * k * d + e * 3 * k * k * j + Math.pow(k, 3) * g, y: Math.pow(e, 3) * a + Math.pow(e, 2) * 3 * k * c + e * 3 * k * k * h + Math.pow(k, 3) * f}
}, snapEnds: function (n, c, j, t) {
    if (Ext.isDate(n)) {
        return this.snapEndsByDate(n, c, j)
    }
    var e = (c - n) / j, b = Math.floor(Math.log(e) / Math.LN10) + 1, l = Math.pow(10, b), d, p, q = Math.round((e % l) * Math.pow(10, 2 - b)), r = [
        [0, 15],
        [10, 1],
        [20, 4],
        [25, 2],
        [50, 9],
        [100, 15]
    ], a = 0, k, h, o, g, s = 1000000000, f = r.length;
    p = Math.floor(n / l) * l;
    if (n == p && p > 0) {
    ...
    for (j in l) {
        if (l.hasOwnProperty(j) && e.stopsRE.test(j)) {
            d = {offset: parseInt(j, 10), color: Ext.draw.Color.toHex(l[j].color) || "#ffffff", opacity: l[j].opacity || 1};
            a.push(d)
        }
    }
    ...
    if (f == "linear") {
        ...
        return{id: k.id, type: f, centerX: k.centerX, centerY: k.centerY, focalX: k.focalX, focalY: k.focalY, radius: k.radius, vector: b, stops: a}
    }
}});
Ext.define("Ext.chart.axis.Axis", {extend: "Ext.chart.axis.Abstract", alternateClassName: "Ext.chart.Axis", requires: ["Ext.draw.Draw"], hidden: false, forceMinMax: false, dashSize: 3, position: "bottom", skipFirst: false, length: 0, width: 0, adjustEnd: true, majorTickSteps: false, nullGutters: {lower: 0, upper: 0, verticalAxis: undefined}, applyData: Ext.emptyFn, getRange: function () {
    ...
    if (d || e) {
        ...
        if (l.length) {
            if (!s.gridLines) {
                s.gridLines = s.chart.surface.add({type: "path", path: l, "stroke-width": s.lineWidth || 1, stroke: s.gridColor || "#ccc"})
            }
...
Ext.define("Ext.rtl.chart.axis.Axis", {override: "Ext.chart.axis.Axis", constructor: function () {
    var a = this, b;
    a.callParent(arguments);
    b = a.position;
...
Ext.define("Ext.chart.axis.Category", {extend: "Ext.chart.axis.Axis", alternateClassName: "Ext.chart.CategoryAxis", alias: "axis.category", isCategoryAxis: true, doConstrain: function () {
...
Ext.define("Ext.chart.axis.Gauge", {extend: "Ext.chart.axis.Abstract", position: "gauge", alias: "axis.gauge", drawAxis: function (p) {
    ...
    if (this.margin >= 0) {
        if (!this.sprites) {
            for (e = 0; e <= k; e++) {
                m = a.add({type: "path", path: ["M", d + (l - c) * n(e / k * f - f), b + (l - c) * j(e / k * f - f), "L", d + l * n(e / k * f - f), b + l * j(e / k * f - f), "Z"], stroke: "#ccc"});
            ...
            g = this.sprites;
            for (e = 0; e <= k; e++) {
                g[e].setAttributes({path: ["M", d + (l - c) * n(e / k * f - f), b + (l - c) * j(e / k * f - f), "L", d + l * n(e / k * f - f), b + l * j(e / k * f - f), "Z"], stroke: "#ccc"}, true)
            }
        }
    }
    this.sprites = g;
    ...
    if (!this.labelArray) {
        for (r = 0; r <= s; r++) {
            w = (r === 0 || r === s) ? 7 : 0;
            k = h ? s - r : r;
            f = q.add({type: "text", text: p(v(l + k / s * (t - l))), x: j + d * c(r / s * u - u), y: g + d * a(r / s * u - u) - w, "text-anchor": "middle", "stroke-width": 0.2, zIndex: 10, stroke: "#333"});
        ...
        for (r = 0; r <= s; r++) {
            w = (r === 0 || r === s) ? 7 : 0;
            k = h ? s - r : r;
            o[r].setAttributes({text: p(v(l + k / s * (t - l))), x: j + d * c(r / s * u - u), y: g + d * a(r / s * u - u) - w}, true)
        }
    }
    this.labelArray = o
}});
Ext.define("Ext.rtl.chart.axis.Gauge", {override: "Ext.chart.axis.Gauge", constructor: function () {
    var a = this;
    a.callParent(arguments);
...
Ext.define("Ext.chart.axis.Numeric", {extend: "Ext.chart.axis.Axis", alternateClassName: "Ext.chart.NumericAxis", type: "Numeric", isNumericAxis: true, alias: "axis.numeric", uses: ["Ext.data.Store"], constructor: function (c) {
...
}, minimum: NaN, maximum: NaN, constrain: true, decimals: 2, scale: "linear", doConstrain: function () {
...
}, position: "left", adjustMaximumByMajorUnit: false, adjustMinimumByMajorUnit: false, processView: function () {
...
Ext.define("Ext.chart.axis.Radial", {extend: "Ext.chart.axis.Numeric", position: "radial", alias: "axis.radial", drawAxis: function (t) {
    ...
    if (!this.sprites) {
        ...
        for (f = 0; f < b; f++) {
            q = a.add({type: "path", path: ["M", e, c, "L", e + o * r(f / b * g), c + o * m(f / b * g), "Z"], stroke: "#ccc"});
            q.setAttributes({hidden: false}, true);
            h.push(q)
        }
    } else {
        ...
        for (d = 0; d < b; d++) {
            h[f + d].setAttributes({path: ["M", e, c, "L", e + o * r(d / b * g), c + o * m(d / b * g), "Z"], stroke: "#ccc"}, true)
        }
    }
    this.sprites = h;
    this.drawLabel()
    ...
    if (!this.labelArray) {
        if (C != "categories") {
            for (E = 1; E <= G; E++) {
                l = B.add({type: "text", text: I(E / G * H), x: n, y: m - g * E / G, "text-anchor": "middle", "stroke-width": 0.1, stroke: "#333"});
        ...
        if (C != "scale") {
            for (D = 0, G = A.length; D < G; D++) {
                s = e(D / G * y) * (g + t);
                r = a(D / G * y) * (g + t);
                l = B.add({type: "text", text: A[D], x: n + s, y: m + r, "text-anchor": s * s <= 0.001 ? "middle" : (s < 0 ? "end" : "start")});
                l.setAttributes({hidden: false}, true);
                x.push(l)
            }
        }
        ...
        if (C != "categories") {
            for (E = 0; E < G; E++) {
                x[E].setAttributes({text: I((E + 1) / G * (H - u) + u), x: n, y: m - g * (E + 1) / G, "text-anchor": "middle", "stroke-width": 0.1, stroke: "#333"}, true)
            }
        }
        if (C != "scale") {
            for (D = 0, G = A.length; D < G; D++) {
                ...
                r = a(D / G * y) * (g + t);
                if (x[E + D]) {
                    x[E + D].setAttributes({type: "text", text: A[D], x: n + s, y: m + r, "text-anchor": s * s <= 0.001 ? "middle" : (s < 0 ? "end" : "start")}, true)
                }
            }
        }
    }
...
Ext.define("Ext.chart.axis.Time", {extend: "Ext.chart.axis.Numeric", alternateClassName: "Ext.chart.TimeAxis", type: "Time", alias: "axis.time", uses: ["Ext.data.Store"], dateFormat: false, fromDate: false, toDate: false, step: [Ext.Date.DAY, 1], constrain: false, constructor: function (b) {
    var c = this, a, d, e;
    c.callParent([b]);
    a = c.label || {};
    e = this.dateFormat;
...
Ext.define("Ext.chart.series.Series", {mixins: {observable: "Ext.util.Observable", labels: "Ext.chart.Label", highlights: "Ext.chart.Highlight", tips: "Ext.chart.Tip", callouts: "Ext.chart.Callout"}, type: null, title: null, showInLegend: true, renderer: function (e, a, c, d, b) {
    return c
}, shadowAttributes: null, animating: false, nullGutters: {lower: 0, upper: 0, verticalAxis: undefined}, nullPadding: {left: 0, right: 0, width: 0, bottom: 0, top: 0, height: 0}, constructor: function (a) {
    var b = this;
    if (a) {
        Ext.apply(b, a)
    }
...
}, initialize: Ext.emptyFn, onRedraw: Ext.emptyFn, eachRecord: function (c, b) {
    var a = this.chart;
    ...
    d.clipBox = e;
    f = {x: (e.x + g.left) - (c.zoom.x * c.zoom.width), y: (e.y + g.bottom) - (c.zoom.y * c.zoom.height), width: (e.width - (g.left + g.right)) * c.zoom.width, height: (e.height - (g.bottom + g.top)) * c.zoom.height};
    d.bbox = f
    ...
    if (c.animating) {
        ...
        c.animating = true;
        return b.animate(Ext.apply(Ext.applyIf(a, c.chart.animate), {callback: function () {
            c.animating = false;
...
Ext.define("Ext.rtl.chart.series.Cartesian", {override: "Ext.chart.series.Cartesian", initialize: function () {
    var a = this;
    a.callParent(arguments);
...
Ext.define("Ext.chart.series.Area", {extend: "Ext.chart.series.Cartesian", alias: "series.area", requires: ["Ext.chart.axis.Axis", "Ext.draw.Color", "Ext.fx.Anim"], type: "area", stacked: true, style: {}, constructor: function (c) {
    this.callParent(arguments);
    ...
    if (e.highlight) {
        e.highlightSprite = a.add({type: "path", path: ["M", 0, 0], zIndex: 1000, opacity: 0.3, lineWidth: 5, hidden: true, stroke: "#444"})
    }
    ...
    for (e = 0; e < p.areasLen; e++) {
        ...
        j = a + e;
        if (!h.areas[e]) {
            h.items[e].sprite = h.areas[e] = d.add(Ext.apply({}, {type: "path", group: m, path: p.paths[e], stroke: b.stroke || n[j % q], fill: n[j % q]}, b || {}))
        }
        f = h.areas[e];
        o = p.paths[e];
        if (c) {
    ...
    return f.chart.surface.add(Ext.apply({type: "text", "text-anchor": "middle", group: g, x: Number(j.point[0]), y: h.y + h.height / 2}, b || {}))
    ...
    var L = this, r = L.chart, C = r.surface, G = r.resizing, K = L.callouts, s = L.items, u = (F == 0) ? false : s[F - 1].point, w = (F == s.length - 1) ? false : s[F + 1].point, c = I.point, z, f, M, J, n, o, b = (l && l.label ? l.label.getBBox() : {width: 0, height: 0}), H = 30, B = 10, A = 3, g, e, h, v, t, D = L.clipRect, m, k;
    if (!b.width || !b.height) {
        return
    }
    if (!u) {
    ...
    if (this._index !== undefined) {
        ...
        if (a.__highlighted) {
            ...
            a.__highlightAnim = new Ext.fx.Anim({target: a, to: {fill: a.__prevFill, opacity: a.__prevOpacity, lineWidth: a.__prevLineWidth}})
        }
    }
...
Ext.define("Ext.chart.series.Bar", {extend: "Ext.chart.series.Cartesian", alternateClassName: ["Ext.chart.BarSeries", "Ext.chart.BarChart", "Ext.chart.StackedBarChart"], requires: ["Ext.chart.axis.Axis", "Ext.fx.Anim"], type: "bar", alias: "series.bar", column: false, style: {}, gutter: 38.2, groupGutter: 38.2, xPadding: 0, yPadding: 10, defaultRotate: {x: 0, y: 0, degrees: 0}, constructor: function (c) {
    ...
    return{bars: z, barsLoc: m, bbox: s, shrunkBarWidth: G, barsLen: r, groupBarsLen: n, barWidth: x, groupBarWidth: f, scale: D, zero: a, padding: R, signed: J / t < 0, minY: J, maxY: t}
}, getPaths: function () {
    ...
    for (ab = 0, H = ad.length; ab < H; ab++) {
        ...
        for (Z = 0, g = 0; Z < v; Z++) {
            ...
            y.push({series: u, yField: T[Z], storeItem: O, value: [O.get(u.xField), ag], attr: r, point: Q ? [r.x + r.width / 2, ag >= 0 ? r.y : r.y + r.height] : [ag >= 0 ? r.x + r.width : r.x, r.y + r.height / 2]});
            if (Y && ae.resizing) {
                p = Q ? {x: r.x, y: J.zero, width: r.width, height: 0} : {x: J.zero, y: r.y, width: 0, height: r.height};
                if (n && (W && !s || !W)) {
                    s = true;
                    for (e = 0; e < q; e++) {
    ...
    if (p == q || p == P || p == j) {
        ...
        if (p == F || p == n) {
            if (K && g) {
                r = J.attr.text;
                J.setAttributes({style: Ext.applyIf((J.attr && J.attr.style) || {}, {"font-weight": "bold", "font-size": "14px"})});
                f = l.getLabelSize(r, J.attr.style);
                a = f.width;
                c = f.height;
                switch (p) {
...
Ext.define("Ext.chart.series.Gauge", {extend: "Ext.chart.series.Series", type: "gauge", alias: "series.gauge", rad: Math.PI / 180, highlightDuration: 150, angleField: false, needle: false, donut: false, showInLegend: false, style: {}, constructor: function (b) {
    this.callParent(arguments);
    var g = this, f = g.chart, a = f.surface, h = f.store, j = f.shadow, d, c, e;
    Ext.apply(g, b, {shadowAttributes: [
        {"stroke-width": 6, "stroke-opacity": 1, stroke: "rgb(200, 200, 200)", translate: {x: 1.2, y: 2}},
        {"stroke-width": 4, "stroke-opacity": 1, stroke: "rgb(150, 150, 150)", translate: {x: 0.9, y: 1.5}},
    ...
    a.customAttributes.segment = function (k) {
        return g.getSegment(k)
    }
}, initialize: function () {
    ...
    if (o) {
        ...
        return{path: [
            ...
            ["A", b.endRho, b.endRho, 0, +t, 1, s, c],
            ["L", u, e],
            ["A", b.startRho, b.startRho, 0, +t, 0, w, g],
            ["Z"]
        ]}
    }
    ...
    for (T = 0, G = r.length; T < G; T++) {
        v = r[T];
        C = A.getAt(T);
        t = Ext.apply({segment: {startAngle: v.startAngle, endAngle: v.endAngle, margin: 0, rho: v.rho, startRho: v.rho * +d / 100, endRho: v.rho}}, Ext.apply(m, g && {fill: g[T % z]} || {}));
    ...
    if (w.needle) {
        u = u * Math.PI / 180;
        if (!w.needleSprite) {
            w.needleSprite = w.chart.surface.add({type: "path", path: ["M", f + (w.radius * +d / 100) * k(u), e + -Math.abs((w.radius * +d / 100) * s(u)), "L", f + w.radius * k(u), e + -Math.abs(w.radius * s(u))], "stroke-width": 4, stroke: "#222"})
        } else {
            if (R) {
                ...
                w.needleSprite.setAttributes({type: "path", path: ["M", f + (w.radius * +d / 100) * k(u), e + -Math.abs((w.radius * +d / 100) * s(u)), "L", f + w.radius * k(u), e + -Math.abs(w.radius * s(u))]})
            }
        }
...
Ext.define("Ext.rtl.chart.series.Gauge", {override: "Ext.chart.series.Gauge", initialize: function () {
    var a = this;
    a.callParent(arguments);
    if (a.chart.getInherited().rtl) {
        a.reverse = true
...
Ext.define("Ext.chart.series.Line", {extend: "Ext.chart.series.Cartesian", alternateClassName: ["Ext.chart.LineSeries", "Ext.chart.LineChart"], requires: ["Ext.chart.axis.Axis", "Ext.chart.Shape", "Ext.draw.Draw", "Ext.fx.Anim"], type: "line", alias: "series.line", selectionTolerance: 20, showMarkers: true, markerConfig: {}, style: {}, smooth: false, defaultSmoothness: 3, fill: false, constructor: function (c) {
    ...
    for (Q = 0; Q < aB; Q++) {
        ...
        if (N) {
            H = aI.getAt(m++);
            if (!H) {
                H = Ext.chart.Shape[Y](z, Ext.apply({group: [V, aI], x: 0, y: 0, translate: {x: +(h || K), y: g || (aw.y + aw.height / 2)}, value: '"' + r + ", " + E + '"', zIndex: 4000}, av));
    ...
    if (aC.animate) {
        ...
        if (D) {
            ...
            ap.onAnimate(ap.fillPath, {to: Ext.apply({}, {path: d, fill: at.fill || w[ap.themeIdx % S], "stroke-width": 0, opacity: ar}, at || {})})
        }
        if (N) {
            m = 0;
            for (Q = 0; Q < aB; Q++) {
    ...
    if (w.chart.animate && !w.chart.resizing) {
        ...
        if (t && w.animation) {
            w.animation.on("afteranimate", function () {
                g.show(true)
            })
        } else {
            g.show(true)
...
Ext.define("Ext.chart.series.Pie", {alternateClassName: ["Ext.chart.PieSeries", "Ext.chart.PieChart"], extend: "Ext.chart.series.Series", type: "pie", alias: "series.pie", accuracy: 100000, rad: Math.PI * 2 / 100000, highlightDuration: 150, angleField: false, lengthField: false, donut: false, showInLegend: false, style: {}, clockwise: false, rotation: undefined, constructor: function (b) {
    this.callParent(arguments);
    ...
    Ext.apply(h, b, {shadowAttributes: [
        {"stroke-width": 6, "stroke-opacity": 1, stroke: "rgb(200, 200, 200)", translate: {x: 1.2, y: 2}},
        {"stroke-width": 4, "stroke-opacity": 1, stroke: "rgb(150, 150, 150)", translate: {x: 0.9, y: 1.5}},
        {"stroke-width": 2, "stroke-opacity": 1, stroke: "rgb(100, 100, 100)", translate: {x: 0.6, y: 1}}
    ]});
    ...
    if (d.startRho !== 0) {
        ...
        return{path: [
            ["M", D, k],
            ["A", d.endRho, d.endRho, 0, 0, 1, v, b],
            ["L", v, b],
            ["A", d.endRho, d.endRho, 0, B, 1, A, f],
            ["L", A, f],
            ["L", C, j],
            ["A", d.startRho, d.startRho, 0, B, 0, w, c],
            ["L", w, c],
            ["A", d.startRho, d.startRho, 0, 0, 0, E, l],
            ["L", E, l],
            ["Z"]
        ]}
    } else {
    ...
    if (f) {
        for (U = 0, F = m.length; U < F; U++) {
            ...
            for (T = 0, A = 0, B = []; T < K; T++) {
                ...
                n = {segment: {startAngle: q.startAngle, endAngle: q.endAngle, margin: 0, rho: q.rho, startRho: A + (o * b / 100), endRho: A + o}, hidden: !q.value && (q.startAngle % r.accuracy) == (q.endAngle % r.accuracy)};
                for (D = 0, B = []; D < aa; D++) {
                    Y = P[D];
                    H = Q[D].getAt(U);
                    if (!H) {
    ...
    for (U = 0, F = m.length; U < F; U++) {
        ...
        for (T = 0, A = 0; T < K; T++) {
            ...
            n = Ext.apply({segment: {startAngle: q.startAngle, endAngle: q.endAngle, margin: 0, rho: q.rho, startRho: A + (o * b / 100), endRho: A + o}, hidden: (!q.value && (q.startAngle % r.accuracy) == (q.endAngle % r.accuracy))}, Ext.apply(g, e && {fill: e[(K > 1 ? T : U) % v]} || {}));
            ...
            x[U] = C;
            if (!y) {
                l = Ext.apply({type: "path", group: w, middle: C.middle}, Ext.apply(g, e && {fill: e[(K > 1 ? T : U) % v]} || {}));
            ...
            if (S) {
                ...
                y._to = n;
                y._animating = true;
                r.onAnimate(y, {to: n, listeners: {afteranimate: {fn: function () {
    ...
    var c = this, e = c.label.calloutLine, a = c.chart.surface.add({type: "path", stroke: (b === undefined ? "#555" : ((e && e.color) || c.getLegendColor(b))), lineWidth: (e && e.width) || 2, path: "M0,0Z", hidden: d});
    return a
    ...
    var z = this, f = z.rad, Z = z.chart, M = Z.resizing, K = z.label, F = K.renderer, m = K.field, e = z.centerX, d = z.centerY, l = E.startAngle, X = E.endAngle, I = E.middle, B = {x: I.x, y: I.y}, O = I.x - e, N = I.y - d, t = {}, J = 1, A = Math.atan2(N, O || 1), r = Ext.draw.Draw.degrees(A), h, U, a, c, Y = (D === "outside"), T = R.attr.calloutLine, C = (T && T.width) || 2, k = (R.attr.padding || 20) + (Y ? C / 2 + 4 : 0), w = 0, u = 0, S, Q, H;
    B.hidden = false;
    ...
    R._from = t;
    if (R.isOutside && T) {
        var n = R.lineSprite, b = V, v = {x: (E.endRho - C / 2) * Math.cos(A) + e, y: (E.endRho - C / 2) * Math.sin(A) + d}, L = {x: B.x, y: B.y}, ab = {};
    ...
    var z = this, o = z.chart, h = z.centerX, g = z.centerY, A = w.middle, b = {x: A.x, y: A.y}, l = A.x - h, j = A.y - g, c = 1, m, f = Math.atan2(j, l || 1), a = (k && k.label ? k.label.getBBox() : {width: 0, height: 0}), v = 20, s = 10, r = 10, q;
    if (!a.width || !a.height) {
        return
    }
    c = w.endRho + v;
    ...
    if (o.animate) {
        ...
        z.onAnimate(k.box, {to: {x: b.x + (l > 0 ? s : -(s + a.width + 2 * r)), y: b.y + (j > 0 ? (-a.height - r / 2) : (-a.height - r / 2)), width: a.width + 2 * r, height: a.height + 2 * r}});
        z.onAnimate(k.label, {to: {x: b.x + (l > 0 ? (s + r) : -(s + a.width + r)), y: b.y + (j > 0 ? -a.height / 4 : -a.height / 4)}})
    } else {
        k.lines.setAttributes({path: ["M", l + h, j + g, "L", b.x, b.y, "Z", "M", b.x, b.y, "l", l > 0 ? s : -s, 0, "z"]}, true);
        k.box.setAttributes({x: b.x + (l > 0 ? s : -(s + a.width + 2 * r)), y: b.y + (j > 0 ? (-a.height - r / 2) : (-a.height - r / 2)), width: a.width + 2 * r, height: a.height + 2 * r}, true);
        k.label.setAttributes({x: b.x + (l > 0 ? (s + r) : -(s + a.width + r)), y: b.y + (j > 0 ? -a.height / 4 : -a.height / 4)}, true)
    }
    for (q in k) {
        k[q].show(true)
    }
    ...
    if (("segment" in w.highlightCfg) && w.items) {
        ...
        for (; s < t; s++) {
            ...
            if (n && n._highlighted) {
                if (h) {
                    g = h.getAt(u.index);
                    o = Ext.apply({translate: {x: 0, y: 0}}, q == "rotate" ? {rotate: {x: g.attr.x, y: g.attr.y, degrees: g.attr.rotation.degrees}} : {});
                    if (g.showOnHighlight) {
                        o.opacity = 0;
                        o.hidden = true
                    }
...
Ext.define("Ext.chart.series.Radar", {extend: "Ext.chart.series.Series", requires: ["Ext.chart.Shape", "Ext.fx.Anim"], type: "radar", alias: "series.radar", rad: Math.PI / 180, showInLegend: false, style: {}, constructor: function (b) {
    this.callParent(arguments);
    ...
    var z = this, o = z.chart, t = o.resizing, w = z.label, s = w.renderer, c = w.field, j = z.centerX, h = z.centerY, b = {x: Number(u.point[0]), y: Number(u.point[1])}, l = b.x - j, k = b.y - h, f = Math.atan2(k, l || 1), m = f * 180 / Math.PI, q, v;
...
Ext.define("Ext.chart.series.Scatter", {extend: "Ext.chart.series.Cartesian", requires: ["Ext.chart.axis.Axis", "Ext.chart.Shape", "Ext.fx.Anim"], type: "scatter", alias: "series.scatter", constructor: function (c) {
    this.callParent(arguments);
    var e = this, f = e.chart.shadow, a = e.chart.surface, d, b;
    Ext.apply(e, c, {style: {}, markerConfig: {}, shadowAttributes: [
        {"stroke-width": 6, "stroke-opacity": 0.05, stroke: "rgb(0, 0, 0)"},
        {"stroke-width": 4, "stroke-opacity": 0.1, stroke: "rgb(0, 0, 0)"},
    ...
    return Ext.chart.Shape[c](b.surface, Ext.apply({}, {x: 0, y: 0, group: e, translate: {x: (f.x + f.width) / 2, y: (f.y + f.height) / 2}}, a))
    ...
    for (o = 0; o < j; o++) {
        ...
        if (k.animate) {
            ...
            for (e = 0; e < q; e++) {
                r = Ext.apply({}, p[e]);
                d = t.renderer(b[e], g.getAt(o), Ext.apply({}, {hidden: false, translate: {x: n.x + (r.translate ? r.translate.x : 0), y: n.y + (r.translate ? r.translate.y : 0)}}, r), o, g);
...
Ext.define("Ext.draw.Matrix", {requires: ["Ext.draw.Draw"], constructor: function (h, g, m, l, k, j) {
    if (h != null) {
        this.matrix = [
            [h, m, k],
            [g, l, j],
...
Ext.define("Ext.rtl.draw.Sprite", {override: "Ext.draw.Sprite", RLM: "\u200F", rtlRe: /[\u0591-\u07FF\uFB1D-\uFDFD\uFE70-\uFEFC]/, transformText: function (b) {
    var a = this;
    if (b && a.surface.isRtl && !Ext.isNumber(b) && a.rtlRe.test(b)) {
        return a.RLM + b
    }
    return a.callParent(arguments)
}});
Ext.define("Ext.draw.Text", {extend: "Ext.draw.Component", uses: ["Ext.util.CSS"], alias: "widget.text", text: "", focusable: false, viewBox: false, autoSize: true, baseCls: Ext.baseCSSPrefix + "surface " + Ext.baseCSSPrefix + "draw-text", initComponent: function () {
    var a = this;
    ...
    for (; c < b; c++) {
        ...
        if (f) {
            e = f.style;
            if (e) {
                Ext.apply(a, {"font-family": e.fontFamily, "font-weight": e.fontWeight, "line-height": e.lineHeight, "font-size": e.fontSize, fill: e.color})
            }
        }
    }
    return a
...
Ext.define("Ext.draw.engine.ImageExporter", {singleton: true, defaultUrl: "http://svg.sencha.io", supportedTypes: ["image/png", "image/jpeg"], widthParam: "width", heightParam: "height", typeParam: "type", svgParam: "svg", formCls: Ext.baseCSSPrefix + "hide-display", generate: function (a, b) {
    b = b || {};
    var e = this, c = b.type, d;
    if (Ext.Array.indexOf(e.supportedTypes, c) === -1) {
        return false
    }
    d = Ext.getBody().createChild({tag: "form", method: "POST", action: b.url || e.defaultUrl, cls: e.formCls, children: [
        {tag: "input", type: "hidden", name: b.widthParam || e.widthParam, value: b.width || a.width},
        {tag: "input", type: "hidden", name: b.heightParam || e.heightParam, value: b.height || a.height},
        {tag: "input", type: "hidden", name: b.typeParam || e.typeParam, value: c},
        {tag: "input", type: "hidden", name: b.svgParam || e.svgParam}
    ]});
...
Ext.define("Ext.draw.engine.Svg", {extend: "Ext.draw.Surface", requires: ["Ext.draw.Draw", "Ext.draw.Sprite", "Ext.draw.Matrix", "Ext.Element"], engine: "Svg", trimRe: /^\s+|\s+$/g, spacesRe: /\s+/, xlink: "http://www.w3.org/1999/xlink", translateAttrs: {radius: "r", radiusX: "rx", radiusY: "ry", path: "d", lineWidth: "stroke-width", fillOpacity: "fill-opacity", strokeOpacity: "stroke-opacity", strokeLinejoin: "stroke-linejoin"}, parsers: {}, minDefaults: {circle: {cx: 0, cy: 0, r: 0, fill: "none", stroke: null, "stroke-width": null, opacity: null, "fill-opacity": null, "stroke-opacity": null}, ellipse: {cx: 0, cy: 0, rx: 0, ry: 0, fill: "none", stroke: null, "stroke-width": null, opacity: null, "fill-opacity": null, "stroke-opacity": null}, rect: {x: 0, y: 0, width: 0, height: 0, rx: 0, ry: 0, fill: "none", stroke: null, "stroke-width": null, opacity: null, "fill-opacity": null, "stroke-opacity": null}, text: {x: 0, y: 0, "text-anchor": "start", "font-family": null, "font-size": null, "font-weight": null, "font-style": null, fill: "#000", stroke: null, "stroke-width": null, opacity: null, "fill-opacity": null, "stroke-opacity": null}, path: {d: "M0,0", fill: "none", stroke: null, "stroke-width": null, opacity: null, "fill-opacity": null, "stroke-opacity": null}, image: {x: 0, y: 0, width: 0, height: 0, preserveAspectRatio: "none", opacity: null}}, createSvgElement: function (d, a) {
    ...
    if (!e.el) {
        ...
        if (Ext.isSafari3) {
            g = e.createSvgElement("rect", {x: -10, y: -10, width: "110%", height: "110%", fill: "none", stroke: "#000"})
        }
        d.appendChild(a);
        if (Ext.isSafari3) {
            d.appendChild(g)
        ...
        e.el.on({scope: e, mouseup: e.onMouseUp, mousedown: e.onMouseDown, mouseover: e.onMouseOver, mouseout: e.onMouseOut, mousemove: e.onMouseMove, mouseenter: e.onMouseEnter, mouseleave: e.onMouseLeave, click: e.onClick, dblclick: e.onDblClick})
    }
    e.renderAll()
...
Ext.define("Ext.draw.engine.SvgExporter", function () {
    var b = /,/g, c = /(-?\d*\.?\d*){1}(em|ex|px|in|cm|mm|pt|pc|%)\s('*.*'*)/, j = /rgb\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)/g, g = /rgba\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,([\d\.]+)\)/g, f, h, e, m, n = function (o) {
        ...
        t = d({d: r, fill: o.fill || "none", stroke: o.stroke, "fill-opacity": o.opacity, "stroke-width": o["stroke-width"], "stroke-opacity": o["stroke-opacity"], "z-index": o.zIndex, transform: s.matrix.toSvg()});
        return"<path " + t + "/>"
    }, text: function (u) {
        ...
        s = d({x: r.x, y: r.y, "font-size": w, "font-family": p, "font-weight": r["font-weight"], "text-anchor": r["text-anchor"], fill: r.fill || "#000", "fill-opacity": r.opacity, transform: u.matrix.toSvg()});
        return"<text " + s + ">" + o + "</text>"
    }, rect: function (p) {
        var o = p.attr, q = d({x: o.x, y: o.y, rx: o.rx, ry: o.ry, width: o.width, height: o.height, fill: o.fill || "none", "fill-opacity": o.opacity, stroke: o.stroke, "stroke-opacity": o["stroke-opacity"], "stroke-width": o["stroke-width"], transform: p.matrix && p.matrix.toSvg()});
        return"<rect " + q + "/>"
    }, circle: function (p) {
        var o = p.attr, q = d({cx: o.x, cy: o.y, r: o.radius, fill: o.translation.fill || o.fill || "none", "fill-opacity": o.opacity, stroke: o.stroke, "stroke-opacity": o["stroke-opacity"], "stroke-width": o["stroke-width"], transform: p.matrix.toSvg()});
        return"<circle " + q + " />"
    }, image: function (p) {
        var o = p.attr, q = d({x: o.x - (o.width / 2 >> 0), y: o.y - (o.height / 2 >> 0), width: o.width, height: o.height, "xlink:href": o.src, transform: p.matrix.toSvg()});
        return"<image " + q + " />"
    }}, a = function () {
...
Ext.define("Ext.draw.engine.Vml", {extend: "Ext.draw.Surface", requires: ["Ext.draw.Draw", "Ext.draw.Color", "Ext.draw.Sprite", "Ext.draw.Matrix", "Ext.Element"], engine: "Vml", map: {M: "m", L: "l", C: "c", Z: "x", m: "t", l: "r", c: "v", z: "x"}, bitesRe: /([clmz]),?([^clmz]*)/gi, valRe: /-?[^,\s\-]+/g, fillUrlRe: /^url\(\s*['"]?([^\)]+?)['"]?\s*\)$/i, pathlike: /^(path|rect)$/, NonVmlPathRe: /[ahqstv]/ig, partialPathRe: /[clmz]/g, fontFamilyRe: /^['"]+|['"]+$/g, baseVmlCls: Ext.baseCSSPrefix + "vml-base", vmlGroupCls: Ext.baseCSSPrefix + "vml-group", spriteCls: Ext.baseCSSPrefix + "vml-sprite", measureSpanCls: Ext.baseCSSPrefix + "vml-measure-span", zoom: 21600, coordsize: 1000, coordorigin: "0 0", zIndexShift: 0, orderSpritesByZIndex: false, path2vml: function (s) {
    ...
    var g = this, a = h.vml, e = a.textpath.style, f = g.span.style, j = g.zoom, k = {fontSize: "font-size", fontWeight: "font-weight", fontStyle: "font-style"}, b, d;
    if (h.dirtyFont) {
        if (c.font) {
